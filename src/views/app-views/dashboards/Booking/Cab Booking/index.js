...
const AddNewCardForm = ({
  ...
  return (
    <Modal
      destroyOnClose={true}
      title={initialVal.id > 0 ? "Edit Cab" : "Add New Cab"}
      open={visible}
      okText="Submit"
      onCancel={onCancel}
      onOk={() => {
...
const Cabllist = () => {
  ...
  const listData = () => {
    const reqeustParam = {};
    try {
      const resp = masterService.getCab(reqeustParam);
      resp
        .then((res) => {
          setList(res.data);
          setListAll(res.data);
  ...
  const closeModal = () => {
    setInitialVal({ id: "", name: "", city_id: "", location: "", });
  ...
  const addEditCab = (values) => {
    let citystatus = values.statusName === true ? 1 : 0;

    if (initialVal.id > 0) {
      const reqeustParam = {
        Cab_id: initialVal.id,
        name: values.name,
      ...
      const resp = masterService.editCab(reqeustParam);
      resp
        .then((res) => {
          if (res.status === 200) {
            listData();
      ...
      const resp = masterService.addCab(reqeustParam);
      resp
        .then((res) => {
          if (res.status === 200) {
          ...
          notification.success({ message: "Cab added successfully." });
          setInitialVal({
            cab_id: "",
            name: "",
            city_id: "",
  ...
  const onOKConfirm = () => {
    const reqeustParam = { cab_id: initialId }; // Update to 'hotel_id'
    const resp = masterService.deleteCab(reqeustParam);
    resp
      .then((res) => {
        if (res.status === 200) {
          setModalVisibleConfirmation(false);
